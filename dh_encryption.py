...
class DiffieHellman:
    ...
    def get_shared_key(self, other_key_str: str) -> str:
    ...
    @staticmethod
    def gen_shared_key_static(local_private_key_str: str, remote_public_key_str: str, group: int = 14) -> str:
...
def encrypt(message, key: str):
    if isinstance(message, list):
        encrypted_messages = []
        for msg in message:
            encrypted_messages.append(xor_encrypt_decrypt(str(msg), key))
        return encrypted_messages
    else:
        return xor_encrypt_decrypt(str(message), key)


def decrypt(encrypted_message, key: str):
    if isinstance(encrypted_message, list):
        decrypted_messages = []
        for msg in encrypted_message:
            decrypted_messages.append(xor_encrypt_decrypt(str(msg), key))
        return decrypted_messages
    else:
        return xor_encrypt_decrypt(str(encrypted_message), key)
...
def decrypt_file(file_binary_data, key_string):
    return encrypt_decrypt_file(file_binary_data, key_string)

def encrypt_file(file_binary_data, key_string):
    return encrypt_decrypt_file(file_binary_data, key_string)

def encrypt_decrypt_file(file_binary_data, key_string):
    with BytesIO(file_binary_data) as df:
        data = xore(df.read(), bytes(key_string, encoding='utf8'))
    return data